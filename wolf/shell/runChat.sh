...
case $1 in
 start)
	...
	cp $PRO_DIR/conf/chat/server.properties $PRO_DIR/conf/server.properties;
	cp $PRO_DIR/conf/chat/log4j.properties $PRO_DIR/conf/log4j.properties;
	
	nohup java -server -Xms512m -Xmx1024m -classpath $PRO_DIR/shell:$PRO_DIR/conf:$PRO_DIR/lib/slf4j-api-1.4.2.jar:$PRO_DIR/lib/slf4j-log4j12-1.4.2.jar:$PRO_DIR/lib/log4j-1.2.14.jar com.youxigu.boot.Root \
	|/usr/local/sbin/cronolog $GAME_LOG_PATH/wolf/wolf.out.%Y%m%d%H >>/dev/null 2>&1 &
	echo $[$! - 1] > $PRO_DIR/chat.pid;
