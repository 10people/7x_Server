...
<%@page import="qxmobile.protobuf.Ranking.GuoGuanInfo"%>
...
if(refresh!=null&&refresh.length()>0){
	...
	for(JunZhu jz:list){
		RankingMgr.inst.resetGuoguanRankRedis(jz);
	}
...
<%
	...
	<table border="1">
		...
		<%
		List<GuoGuanInfo> junList = resp.getGuoguanListList();
		for(GuoGuanInfo jz:junList){
		%>
		<tr>
			<td><%=jz.getRank() %></td>
			<td><%=jz.getJunZhuId() %></td>
			<td><%DecimalFormat format=new DecimalFormat("#,##0.00");%><%=format.format(Redis.getInstance().zscore(RankingMgr.GUOGUAN_RANK+"_"+guojiaId, jz.getJunZhuId()+""))  %></td>
			<td><%=jz.getGuojiaId() %></td>
			<td><%=jz.getName() %></td>
			<td><%=jz.getPutong() %></td>
			<td><%=jz.getChuanqi() %></td>
			<td><%=jz.getStarCount() %></td>
