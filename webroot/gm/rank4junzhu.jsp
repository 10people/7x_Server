...
if(refresh!=null&&refresh.length()>0){
	...
	for(JunZhu jz:list){
		RankingMgr.inst.resetJunzhuRankRedis(jz);
	}
...
<%
	...
	<table border="1">
		...
		<%
		List<JunZhuInfo> junList = resp.getJunListList();
		for(JunZhuInfo jz:junList){
		%>
		<tr>
			<td><%=jz.getRank() %></td>
			<td><%=jz.getJunZhuId() %></td>
			<td><%DecimalFormat format=new DecimalFormat("#,##0.00");%><%=format.format(Redis.getInstance().zscore(RankingMgr.JUNZHU_RANK+"_"+guojiaId, jz.getJunZhuId()+"")) %></td>
