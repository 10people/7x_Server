...
<%
	...
	if(rankType!=0){
		...
		case 2:// 联盟榜
			if(name==null){
				response.sendRedirect("rank4lianmeng.jsp?rankType=2&pageNo="+pageNo+"&guojiaId="+guojiaId+"");
			}else{
		...
		case 201:// 联盟榜（按声望排序）
			if(name==null){
				response.sendRedirect("rank4lianmeng.jsp?rankType=201&pageNo="+pageNo+"&guojiaId="+guojiaId+"");
			}else{
...
<p>
	<h3>参数设置</h3>
	<p>
	每页条目：<input type="text" name="pageSize" value="<%=RankingMgr.PAGE_SIZE%>"/>|
	筛选最大数量：<input type="text" name="junRange" value="<%=RankingMgr.RANK_MAXNUM%>"/>|
	君主最低等级：<input type="text" name="junMinLevel" value="<%=RankingMgr.RANK_MINLEVEL%>"/>
	</p>
	<p>
...
<p>
	<h3>正常刷新加载</h3>
	<p>
	输入请求的页码：<input type="number" name="pageNo"/></p>
	<p>
	输入国家id：<input type="number" name="guojiaId"/></p>
	<input name="rankType" type="radio" value="1">君主榜
	<input name="rankType" type="radio" value="2">联盟榜
	<input name="rankType" type="radio" value="201">联盟榜(按声望)
	<input name="rankType" type="radio" value="3">百战榜
	<input name="rankType" type="radio" value="4">过关榜
...
<p>
	<h3>获取排名</h3>
	<p>
	输入id：<input type="text" name="queryId"/>
	输入国家id：<input type="text" name="queryGjId"/>
	</p>
	<input name="queryType" type="radio" value="1">君主榜
	<input name="queryType" type="radio" value="2">联盟榜
	<input name="queryType" type="radio" value="201">联盟榜(按声望)
	<input name="queryType" type="radio" value="3">百战榜
	<input name="queryType" type="radio" value="4">过关榜
	<input type="submit" value="查看"/>
</p>
</form>
<%
...
out("(排行类型: 战力排行:1; 联盟排行： 2)");%>
 ...
 <br/> 
 <br/>
    排行类型 <input type="text" name="typea" value="<%=typea%>">&nbsp;
...
typea = request.getParameter("typea");
    ...
    if(rankmin != null && rankmin.length()>0 && rankmax != null && rankmax.length()>0 && typea != null){
    	...
    	if(rmin > rmax){
    		...
    		String info = null;
    		switch(type){
    		case 1: diffType = RankingMgr.zhanliRank;
    		info = "战力";
    		  break;
    		case 2: diffType = RankingMgr.lianMengLevel;
    		info = "联盟等级";
    		break;
    		}
    		if(diffType != null){
    	           ...
    	           if(map == null || map.size() ==0){ 
    	               ...
    	               if(type == 1){
	    	               for(Map.Entry<String, Double> entry: map.entrySet()){
	    	                   ...
	    	                   trS();
	    	                   td(idN.split("_")[0]); td(rmin++); td(idN.split("_")[1]);td(zhanli);
	    	                   trE();
	    	               }
    	               }else if(type == 2){
    	            	   for(Map.Entry<String, Double> entry: map.entrySet()){
                               ...
                               trS();
                               td(idN); td(rmin++); td(alncBean.name);td(level);
                               trE();
    	               }
    	               tableEnd();
    	           }
...
for(int i=0;i<=7;i++){
	List<AllianceBean> lianmengDayRank = RankingMgr.inst.getLianmengSWRank(RankingMgr.LIANMENG_SW_DAY_RANK, i);
	List<AllianceBean> lianmengLastDayRank = RankingMgr.inst.getLianmengSWRank(RankingMgr.LIANMENG_SW_LAST_DAY_RANK, i);
	List<AllianceBean> lianmengWeekRank = RankingMgr.inst.getLianmengSWRank(RankingMgr.LIANMENG_SW_WEEK_RANK, i);
	List<AllianceBean> lianmengLastWeekRank = RankingMgr.inst.getLianmengSWRank(RankingMgr.LIANMENG_SW_LAST_WEEK_RANK, i);
...
for(int i=1;i<=7;i++){
	...
	<table border="1" style="margin-left:10px;float:left">
	<%if(i==0){ %>
		<tr><th colspan="3">联盟今日日榜总榜</th></tr>
	<%} else{%>
		<tr><th colspan="3"><%=HeroService.getNameById(""+i) %>国联盟今日日榜</th></tr>
	<%} %>
	<tr>
		<th>联盟id</th>
		<th>联盟名</th>
...
for(int i=1;i<=7;i++){
	...
	<table border="1" style="margin-left:10px;float:left">
	<%if(i==0){ %>
		<tr><th colspan="3">联盟昨日日榜总榜</th></tr>
	<%} else{%>
		<tr><th colspan="3"><%=HeroService.getNameById(""+i) %>国联盟昨日日榜</th></tr>
	<%} %>
	<tr>
		<th>联盟id</th>
		<th>联盟名</th>
...
for(int i=1;i<=7;i++){
	...
	<table border="1" style="margin-left:10px;float:left">
	<%if(i==0){ %>
		<tr><th colspan="3">联盟本周榜总榜</th></tr>
	<%} else{%>
		<tr><th colspan="3"><%=HeroService.getNameById(""+i) %>国联盟本周周榜</th></tr>
	<%} %>
	<tr>
		<th>联盟id</th>
		<th>联盟名</th>
...
for(int i=1;i<=7;i++){
	...
	<table border="1" style="margin-left:10px;float:left">
	<%if(i==0){ %>
		<tr><th colspan="3">联盟上周周榜总榜</th></tr>
	<%} else{%>
		<tr><th colspan="3"><%=HeroService.getNameById(""+i) %>国联盟上周周榜</th></tr>
	<%} %>
	<tr>
		<th>联盟id</th>
		<th>联盟名</th>
