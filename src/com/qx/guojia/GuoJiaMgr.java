...
public class GuoJiaMgr  extends EventProc implements Runnable{
	...
	public void getGongjinAwardResp(int id, Builder builder, IoSession session) {
		JunZhu jz = JunZhuMgr.inst.getJunZhu(session);
		if (jz == null) {
			log.error("请求领取贡金奖励出错：君主不存在");
			return;
		}
		long jzId=jz.id;
		log.info("君主{}请求领取贡金奖励",jzId);
		ResourceGongJin gongjinBean =HibernateUtil.find(ResourceGongJin.class, jzId);
		...
		if(gongjinBean.getDayAwardTime!=null){
			...
			if(isSameDay){
				log.error("请求领取贡金奖励出错：{}已领取本日奖励", jzId);
				resp.setResult(60);
				resp.setAward("");
				resp.setGuojiRank(-1);
				resp.setLianMengRank(-1);
		...
		if (aBean == null) {
			log.error("请求捐献贡金出错：{}的联盟{}不存在", jzId,apBean.lianMengId);
			resp.setResult(50);
			resp.setAward("");
			resp.setGuojiRank(-1);
			resp.setLianMengRank(-1);
		...
		log.info("{}请求领取每日捐献贡金奖励,上一日国家排名--{}，上一日联盟在国家中的声望排行--{}", jzId,guojiRank,lianMengRank);
		...
		if(award==null){
			log.error("{}请求领取贡金奖励出错,未找到奖励配置", jzId);
			resp.setResult(40);
			resp.setAward("");
			resp.setGuojiRank(-1);
			resp.setLianMengRank(-1);
	...
	public boolean isDayAward(	ResourceGongJin gongjinBean){
		if(gongjinBean==null){
			log.info("君主没有国家贡金每日奖励可领");
			return false;
		}
		boolean isSameDay = DateUtils.isSameDay(gongjinBean.getDayAwardTime);	
		if(isSameDay){
			log.info("君主国家贡金每日奖励可领今日已领取过");
			return false;
		}
		if(gongjinBean.lastJX<CanShu.LUEDUO_HAND_DAYMINNUM){
		...
		if (alBean == null) {
			log.info("君主{}没有国家贡金每日奖励可领，无联盟", gongjinBean.junzhuId);
			return false;
		}
		AllianceBean aBean = HibernateUtil.find(AllianceBean.class, alBean.lianMengId);
		if (aBean == null) {
			log.info("君主{}没有国家贡金每日奖励可领，联盟{}不存在", gongjinBean.junzhuId,alBean.lianMengId);
			return false;
		}
		Integer guojiRank=(int) RankingMgr.inst.getRankById(RankingMgr.GUOJIA_DAY_LAST_RANK, aBean.country);
		...
		log.info("君主{} 国家贡金每日奖励判断国家排名---{}，联盟排名---{}", gongjinBean.junzhuId,guojiRank,lianMengRank);
		List<LueduoUnionRank> awardList=dayAwardMap.get(guojiRank);
		String award=null;
		for (LueduoUnionRank dayAward : awardList) {
		...
		if(award==null){
			log.error("君主{}国家上缴贡金玩法无每日奖励，未找到奖励配置", gongjinBean.junzhuId);
			return false;
		}
		log.info("君主{}国家上缴贡金玩法有每日奖励", gongjinBean.junzhuId);
		return true;
...
//		List<GuoJiaBean> gjLastRankList=RankingMgr.inst.getGuojiaLastWeekRank();
		...
		mainResp.setTodayGive(gongjinBean.todayJX);
		mainResp.setThisWeekGive(gongjinBean.thisWeekJX);
		mainResp.setHateGuoId1(gjBean.diDuiGuo_1);
		mainResp.setHateGuoId2(gjBean.diDuiGuo_2);
		mainResp.setMyGongJin(gongjinBean.gongJin);
		int juanXianTimes=gongjinBean.todayJXTimes+1;
		ShangJiaoTemp sj=shangJiaoMap.get(juanXianTimes);
		int needGongjin=0;
		if(sj!=null){
		...
		mainResp.setShouldGive(needGongjin);
		//有每日奖励为1 没有每日奖励为0
