...
public class YouXiaMgr extends EventProc{
	...
	public void initData() {
		...
		for (YouxiaNpcTemp npcTemp : youxiaNpcTempList) {
			...
			List<YouxiaNpcTemp> npcTempList = youxiaNpcMapByNpcId
					.get(npcTemp.npcId);
			if (npcTempList == null) {
	...
	public void battleOverReport(int id, IoSession session, Builder builder) {
		...
		if (pveTemp == null) {
			logger.error("游侠战斗结算失败，找不到游侠章节:{}配置错误，zhangJieId:{}", zhangJieId);
			return;
		}
		...
		int getTongbi = 0;
		int getExp = 0;
		if (result == 1) { // 只有胜利才会给奖励
			//怪物掉落奖励
			Map<Integer, List<AwardTemp>> npcDropAwardMap = dropAwardMapBefore.get(junZhu.id);
			int deadNum = Math.min(droppenList.size(), pveTemp.maxNum);
			for (int i = 0; i < deadNum; i++) {
				Integer npcPos = droppenList.get(i);
				int deadTimes = 0;
				if(npcDeadTimesMap.containsKey(npcPos)) {
					deadTimes += 1;
			...
			//配置的关卡奖励
			List<AwardTemp> guanQiaAwardList = AwardMgr.inst.getHitAwardList(
					pveTemp.awardId, ",", "=");
			...
			if (yxBean == null) {
				...
				// 主线任务
				EventMgr.addEvent(ED.finish_youxia_x, new Object[] { junZhu.id});
				// 每日任务：完成1次游侠活动
				EventMgr.addEvent(ED.DAILY_TASK_PROCESS,
						new DailyTaskCondition(junZhu.id,
								DailyTaskConstants.youXia_activity, 1));
			}
			// 游侠关卡记录
			YouXiaRecord yxRecord = HibernateUtil.find(YouXiaRecord.class, " where junzhuId="+junZhu.id +" and guanQiaId="+ zhangJieId);
			if(yxRecord == null) {
				yxRecord = new YouXiaRecord();
				yxRecord.setId(TableIDCreator.getTableID(YouXiaRecord.class, 1));
				yxRecord.setGuanQiaId(zhangJieId);
				yxRecord.setJunzhuId(junZhu.id);
				yxRecord.setScore(score);
				HibernateUtil.insert(yxRecord);
			} else {
				int before = yxRecord.getScore();
				int nowScore = Math.max(before, score);//跟上次比存储数量较大的
				nowScore = Math.min(nowScore, pveTemp.maxNum);//最大的数量不能超过配置表的最大数量
				yxRecord.setScore(nowScore);
				HibernateUtil.save(yxRecord);
			}
			if(score >= pveTemp.maxNum) {
				EventMgr.addEvent(ED.YOU_XIA_SUCCESS, new Object[]{junZhu, zhangJieId});
			}
		}
		...
		logger.info("游侠战斗结算成功，{}:{}战斗结束，关卡{}，droppenList.size:{} 铜币{}，经验{}，结果{}",junZhu.id,junZhu.name,zhangJieId,
				droppenList.size(),getTongbi,getExp,result);
		switch(pveTemp.bigId){
		case 1:
			ActLog.log.LootRich(junZhu.id, junZhu.name, ActLog.vopenid, "", zhangJieId, result==1?1:2, 1, getTongbi);
			break;
		case 2:
			ActLog.log.KillRobber(junZhu.id, junZhu.name, ActLog.vopenid, "", zhangJieId, result==1?1:2, 1, 0, 0);
			break;
		case 3:
			ActLog.log.KillRebelArmy(junZhu.id, junZhu.name, ActLog.vopenid, "", zhangJieId, result==1?1:2, 1, 0, 0);
			break;
		}
		...
		for (AwardTemp award : getAwardList) {
			logger.info("游侠战斗结算奖励，君主:{} 得到奖励 awardId:{}, 类型:{},id:{},数量:{}",
					junZhu.id,award.getAwardId(),award.getItemType(),award.getItemId(),award.getItemNum());
	 ...
	 * @param npcAwardList			该npc掉落物品序列
	 * @param deadTimes				该npc死亡次数
	 * @param isLoop				该npc是否是重复掉落
	 * @return
	 */
	...
	public void battleInitRequest(int id, IoSession session, Builder builder) {
		...
		if (!yxOpen.OpenDay.contains(String.valueOf(dayNum))) {
			logger.error("游侠战斗信息请求失败，本日未开放该活动type:{}玩法。今日星期{}, 要求:{}", 
					pveTemp.bigId,dayNum, pveTemp.openDay);
			PveMgr.inst.sendZhanDouInitError(session, "活动今日未开放");
			return;
		}
		...
		if (yxBean == null) {
			logger.error("游侠战斗信息请求失败,youxiaBean初始化失败， junzhuId:{} type:{}", junZhu.id,
					pveTemp.bigId);
		...
		if(yxBean.lastBattleTime != null) {
			int intervalTime = (int) ((date.getTime() - yxBean.lastBattleTime
					.getTime()) / 1000);
		...
		if (junZhu.level < pveTemp.monarchLevel) {
			logger.error("游侠战斗信息请求失败，君主等级:{}没有达到该关卡:{}要求等级:{}", junZhu.level, pveTemp.id,
					pveTemp.monarchLevel);
		...
		if (npcList == null || npcList.size() == 0) {
			logger.error("游侠战斗信息请求失败，章节怪物配置为空，zhangjieId:{}, npcId:{}", zhangJieId,
					pveTemp.npcId);
		...
		for (YouxiaNpcTemp npcTemp : npcList) {
			...
			if (nodeProfession == null) {
				logger.error("游侠战斗信息请求失败，"
						+ "nodeProfession与npcTemp的Profession值不一致，npcTemp.Profession:{}",
						npcTemp.profession);
				continue;
			}
		...
		PveMgr.inst.fillJunZhuDataInfo(resp, session, selfs, junZhu, 1, zuheId, selfTroop);
	...
	public void youxiaInfoRequest(int id, IoSession session, Builder builder) {
		...
		for (Map.Entry<Integer, YouxiaOpenTime> entry : youxiaOpenTimeMap
				.entrySet()) {
			...
			if (beanInfo.lastBattleTime == null || beanInfo.times <= 0) {
				youxiaInfo.setRemainColdTime(0);
			} else {
				int coldTime = 0;
				int intervalTime = (int) ((date.getTime() - beanInfo.lastBattleTime
						.getTime()) / 1000);
			...
			youxiaInfo.setOpen(isOpen(openTimeCfg, beanInfo)); 
	...
	private boolean isOpen(YouxiaOpenTime openTimeCfg, YouXiaBean beanInfo) {
		...
		if (openDay.contains(String.valueOf(nowDay))) {
			...
			// 则是开放并判断是否要刷新次数
			if (nowHour > hourCfg
					|| (nowHour == hourCfg && nowMinutes >= minutescfg)) {
				open = true;
	...
	public void timesInfoRequest(int id, IoSession session, Builder builder) {
		...
		Date date = new Date();
		Date lastBuyTime = yxBean.lastBuyTime == null ? date
				: yxBean.lastBuyTime;
		//change 20150901
		if(DateUtils.isTimeToReset(lastBuyTime, CanShu.REFRESHTIME_PURCHASE)){
			yxBean.buyTimes = 0;
		}
		int maxBuyTimes = VipMgr.INSTANCE.getValueByVipLevel(junZhu.vipLevel,
				VipData.youXiaTimesGet);
		Purchase purchaseCfg = PurchaseMgr.inst.getPurchaseCfg(
				PurchaseConstants.YOUXIA_TIMES, yxBean.buyTimes + 1);
		if (purchaseCfg == null) {
	...
	public void buyTimes(int id, IoSession session, Builder builder) {
		...
		logger.info("游侠购买次数失败，君主:{}购买了游侠玩法次数，type:{}，花费元宝:{}", 
	...
	public void saoDang(int id, IoSession session, Builder builder) {
		...
		YouXiaRecord yxRecord = HibernateUtil.find(YouXiaRecord.class, " where junzhuId="+junZhu.id +" and guanQiaId="+ guanQiaId);
		if(yxRecord != null){
			String awardList = "";
 			...
 			if(!awardList.equals("")) {
 				List<AwardTemp> awards = AwardMgr.inst.parseAwardConf(awardList, "#", ":");
 				PveSaoDangAward.Builder awardResp = PveSaoDangAward.newBuilder();
 				int tongBiTemp = 0;
 				int expTemp = 0;
 				for(AwardTemp award : awards) {
 					SaoDangAwardItem.Builder awardItem = SaoDangAwardItem.newBuilder();
 					awardItem.setItemType(award.getItemType());
 					awardItem.setItemId(award.getItemId());
 					awardItem.setItemNum(award.getItemNum());
 					if (award.getItemId() == AwardMgr.ITEM_TONGBI_ID) {// 铜币
 						...
 						awardResp.addAwardItems(awardItem);
 						AwardMgr.inst.giveReward(session, award, junZhu, false);
 				...
 				logger.info("游侠扫荡奖励未命中，君主:{},章节:{},最好成绩:{}", junZhu.id, guanQiaId, yxRecord.getScore());
 				PveSaoDangAward.Builder awardResp = PveSaoDangAward.newBuilder();
	...
	public void requestGuanQiaInfo(int id, IoSession session, Builder builder) {
		...
		if(yxRecord == null || yxRecord.getScore() <= 0 || coldTime > 0) {
			isSaoDang = false;
		}
	...
	public void isCanBattleYouXia(JunZhu junZhu, IoSession session){
		...
		for(Integer zhangJieId: youxiaPveTempMap.keySet()){
			...
			FunctionID.pushCanShangjiao(junZhu.id, session, FunctionID.youXiaStatus);
			return;
		}
	}
	...
	public void proc(Event evt) {
		...
		case ED.REFRESH_TIME_WORK:
			...
			boolean isOpen=FunctionOpenMgr.inst.isFunctionOpen(FunctionID.youXia, jz.id, jz.level);
			if(!isOpen){
				logger.info("君主：{}--游侠：{}的功能---未开启,不推送",jz.id,FunctionID.youXia);
				break;
			}
