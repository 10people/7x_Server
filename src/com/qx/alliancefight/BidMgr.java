...
public class BidMgr {
	...
	/**揭晓开始*/
	public static String city_war_preparation_startTime = "18:00";
	/**战斗开始*/
	public static String city_war_fighting_startTime = "20:00";
	/**战斗结束*/
	public static String city_war_fighting_endTime = "23:00";
	/**宣战消息清空*/
	...
	public void bid(int id, IoSession session, Builder builder) {
		...
		int type = jczmap.get(city).getType();
		if(type == 1){
	...
	public void bid(int id,IoSession session,JunZhu jz,int city,int hufu) {
		...
		//是否是宣战时段
		if(!DateUtils.isInDeadline4Start(city_war_declaration_startTime,city_war_preparation_startTime) && jczmap.get(city).getType() == 1){ //不在宣战时间段内(普通)
			log.error("不在宣战时间段内");
			cwop.setResult(city_war_operate_result2);
			msgSend(PD.S_CITYWAR_OPERATE_RESP,session,cwop);
			return;
		}else if(!DateUtils.isInDeadline4Start(city_war_declaration_startTime,city_war_fighting_startTime) && jczmap.get(city).getType() == 2){//不在宣战时间段内(野城)
			log.error("不在宣战时间段内");
			cwop.setResult(city_war_operate_result2);
			msgSend(PD.S_CITYWAR_OPERATE_RESP,session,cwop);
			return;
		...
		if(jczmap.get(city).getType() == 1){ //普通城池需要校验
		...
		}else if(jczmap.get(city).getType() == 2){ //野城只能对一个野城宣战
		...
		//校验联盟等级
		int needLmLv = jczmap.get(city).getAllianceLv();
		...
		//校验虎符
		if(jczmap.get(city).getType() == 2){ //野城虎符读配置
		...
		if (bean == null) {
			...
			bean.cityId = city;
			bean.type = jczmap.get(city).getType();
	...
	public void normalCityFightInfo(int id,IoSession session,Builder builder){
		...
		//查询所有城池状态信息
		List<CityBean> citiesState = HibernateUtil.list(CityBean.class,"where 1=1");
		...
		for(JCZCity jczCity:cityListSetting){
			...
			cityInfo.setCityId(jczCity.getId()); //设置cityId
			int state2 = 0; //0-无、1-宣战、2-防守,3- 进攻状态
			List<BidBean> myBidList = HibernateUtil.list(BidBean.class, "where cityId=" + jczCity.id + " and bidTime>'" + dayStart() + "' and bidTime<'" + dayEnd() +"'");
			if(citiesStateMap.containsKey(jczCity.getId()) && citiesStateMap.get(jczCity.getId()).lmId > 0){ //玩家占领
				if(citiesStateMap.get(jczCity.getId()).lmId == myAlliance.id){ //乙方占领
					...
					myCityCount++;
					if(myBidList != null && myBidList.size() >= 0){
						state2 = 2;
					}
				}else{
					AllianceBean allianceBean = HibernateUtil.find(AllianceBean.class,citiesStateMap.get(jczCity.getId()).lmId);
					if(allianceBean == null){ //联盟被解散
						CityBean cityBean = citiesStateMap.get(jczCity.getId());
			...
			if(myBidList != null && myBidList.size() > 0){
				for (BidBean bidBean : myBidList) {
					if(bidBean.lmId == myAlliance.id){ //我对该城池宣战
						state2 = 1;
					}
				}
			}
			CityBean cityBean = HibernateUtil.find(CityBean.class,jczCity.id);
			...
			if(DateUtils.isInDeadline4Start(city_war_preparation_startTime,city_war_fighting_startTime)){ 
				if(cityBean.atckLmId > 0 && cityBean.atckLmId != myAlliance.id){
					AllianceBean atckalliance = HibernateUtil.find(AllianceBean.class,cityBean.atckLmId);
			...
			if(!DateUtils.isInDeadline4Start(city_war_declaration_startTime,city_war_fighting_endTime)){ //其他阶段
				state2 = 0;
			}
	...
	public void wildCityFightInfo(int id,IoSession session,Builder builder){
		...
		for(JCZCity jczCity:cityListSetting){
			...
			cityInfo.setCityId(jczCity.getId()); //设置cityId
			//设置城池状态
			WildCityBean wildCityBean = HibernateUtil.find(WildCityBean.class,"where cityId=" + jczCity.getId() + " and lmId=" + myAlliance.id);
			if(wildCityBean != null &&  wildCityBean.winTime != null){
				cityInfo.setCityState(city_state_conquer); 
			}else{
				cityInfo.setCityState(city_state_not_conquer);
			}
			if(DateUtils.isInDeadline4Start(city_war_fighting_startTime,city_war_fighting_endTime)){
				List list = HibernateUtil.list(EnterWarTimeBean.class,"where cityId=" + jczCity.id + " and enterTime>'" + dayStart() + "' and enterTime<'" + dayEnd() + "'");
				if(list == null){
					cityInfo.setLmNum(0);
				}else{
					cityInfo.setLmNum(list.size());
				}
			}
			BidBean bidBean = HibernateUtil.find(BidBean.class,"where cityId=" + jczCity.id + " and lmId=" + myAlliance.id + " and bidTime>'" + dayStart() + "' and bidTime<'" + dayEnd() + "'");
			if(bidBean != null){
	...
	public void cityWarBidPageInfo(int id,IoSession session,Builder builder){
		...
		if(cityBean != null){
			if(cityBean.lmId > 0){
				AllianceBean alliance = HibernateUtil.find(AllianceBean.class, cityBean.lmId);
				String name = "";
				String aName = "";
				if(alliance == null) {
				}else{
				...
				resp.setGeneral(name);
				resp.setAllianceName(aName);
			}
		}
		//进入战场联盟数
		if(DateUtils.isInDeadline4Start(city_war_fighting_startTime,city_war_fighting_endTime)){
			List<EnterWarTimeBean> list = HibernateUtil.list(EnterWarTimeBean.class,"where cityId=" + cityId + " and enterTime>'" + dayStart() + "' and enterTime<'" + dayEnd() + "'");
			if(list == null){
				resp.setLmNum(0);
			}else{
...
//		calendar.setTime(dayStart());
	...
	public void enterCityWar(int id,IoSession session,Builder builder){
		...
		//校验时间段
		if(!DateUtils.isInDeadline4Start(city_war_preparation_startTime,city_war_fighting_endTime)){ //非战斗时段,2016-5-21 16:03:28 策划改为揭晓时段就可以进入站场
			result = 2; //2-非战斗时段
			resp.setResult(result);
			msgSend(PD.S_CITYWAR_OPERATE_RESP,session,resp);
			return;
		}else{
			if(jczmap.get(cityId).getType() == 1){ //普通城池需要校验
				//没有敌人宣战
				if(cityBean != null && cityBean.lmId > 0){
					if(cityBean.lmId == aBean.id){
						List<BidBean> bidList = HibernateUtil.list(BidBean.class, "where cityId=" + cityId + " and bidTime>'" + dayStart() + "' and bidTime<'" + dayEnd() +"'");
	...
	public void getCityWarReward(int id,IoSession session,Builder builder){
		...
		AwardMgr.inst.giveReward(session,jczmap.get(lmzAwardBean.cityId).getAward(),jz);
		resp.setResult(0);
		//修改状态
	...
	public void bidJieSuan(){
		...
		for (JCZCity jczCity : citylist) {
			...
			if(beans != null && beans.size() > 0){
				...
				for (BidBean bidBean : beans) {
					AllianceBean allianceBean = HibernateUtil.find(AllianceBean.class,bidBean.lmId);
					if(allianceBean == null) continue;//特殊处理联盟不存在情况
					if(atckLmid <= 0){
						atckLmid = bidBean.lmId; //只存一次
					}
		...
		for (Map.Entry<Integer, Integer> entry : hufuMap.entrySet()) {  
			AllianceBean allianceBean = HibernateUtil.find(AllianceBean.class,entry.getKey());
	...
	public void saveLog(int lmId1,int lmId2,int cityId,Date ot){
		...
		AllianceBean allianceBean1 = HibernateUtil.find(AllianceBean.class,lmId1);
		...
		if(lmId2 > 0){
			AllianceBean allianceBean2 = HibernateUtil.find(AllianceBean.class,lmId2);
			...
			int npcId = jczmap.get(cityId).getNpcId();
			cwLogBean.lmName2 = jczNpcmap.get(npcId).name;
	...
	public void regularClearData(){
		//清理竞拍记录
		List<BidBean> bidList = HibernateUtil.list(BidBean.class,"where bidTime<"+ new Date()); 
		if(bidList != null){
			for (BidBean bidBean : bidList) {
				HibernateUtil.delete(bidBean);	
			}
		}
		//重置结算数据
		...
		//重置进入战场数据
		List<EnterWarTimeBean> enterWarTimeList = HibernateUtil.list(EnterWarTimeBean.class,"where enterTime<"+ new Date());
