...
import com.manu.dynasty.template.SelfBuilder;
...
public class DataLoader {
	...
	public void load() {
		...
		try {
			logger.info("file: {} {}", GameServer.confFileBasePath, this.config);
			...
			for (Object n : nodes) {
				...
				for (Object o : dataList) {
					...
					if(o instanceof SelfBuilder) {
						SelfBuilder sb = (SelfBuilder)o;
						sb.build();
					}
				}
				files.add(f);
			...
			TempletService.getInstance().templetMap = dataMap;
	...
	public List<?> loadFromStream(InputStream resourceAsStream) 
		...
		SAXReader reader = new SAXReader();
		Document doc = reader.read(new InputStreamReader(resourceAsStream, "utf-8"));
		Element dataSet = (Element) doc.selectNodes("/dataset").get(0);
		List<?> nodes = dataSet.elements();
		// get clazz
		String className = this.packageName + ((Element) nodes.get(0)).getName();
		try {
